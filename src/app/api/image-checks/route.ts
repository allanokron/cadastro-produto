...
export async function POST(request:Request){
  try{
    ...
    const batch=pending.slice(0,300);const results:{skuKey:string;signature:string;availableUrl:string|null}[]=[];
    for(let start=0;start<batch.length;start+=30){
      results.push(...await Promise.all(batch.slice(start,start+30).map(async(item)=>({...item,availableUrl:await firstAvailable(item.urls)}))));
    }
