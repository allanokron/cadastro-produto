...
export function LoginForm() {
  ...
  async function submit(event: React.FormEvent<HTMLFormElement>) {
    ...
    router.push(result.mustChangePassword ? "/conta" : "/dashboard"); router.refresh();
  }
  return <form onSubmit={submit} className="space-y-4">
    ...
    <div><label className="label" htmlFor="password">Senha</label><input className="input" id="password" name="password" type="password" defaultValue="Admin" autoComplete="current-password" required /></div>
