...
function productCardHtml(item: PromoItem) {
  const img = item.firstImageUrl
    ? `<img src="${escapeHtml(item.firstImageUrl)}" style="width:100%;height:180px;object-fit:contain;background:#f9fafb;border-radius:8px 8px 0 0;" crossorigin="anonymous" />`
  ...
  const minQty = item.minQty ? `<div style="font-size:12px;color:#666;margin-top:4px;">Mín: ${escapeHtml(item.minQty)} un</div>` : "";
  ...
  return `
    <div style="background:#fff;border:1px solid #e5e7eb;border-radius:10px;overflow:hidden;display:flex;flex-direction:column;box-shadow:0 2px 8px rgba(0,0,0,0.06);">
      ...
      <div style="padding:10px 12px;">
        <div style="font-size:13px;font-weight:700;color:#14181f;line-height:1.3;overflow:hidden;text-overflow:ellipsis;white-space:nowrap;" title="${escapeHtml(item.name)}">${escapeHtml(item.name)}</div>
        <div style="font-size:20px;font-weight:900;color:#1fadad;margin-top:6px;">${price}</div>
        ${minQty}
      </div>
    </div>`;
}
